 ...
 * validateBaseAddress
 *
 * @param {String} stacksAddress - the BASE address to validate
 */
...
export const microToBase = (amountInMicroStacks: string | number): number => {
...
export const microToStacksFormatted = (amountInMicroStacks: string | number): string => {
  return formatStacksAmount(microToBase(amountInMicroStacks));
};

/**
 * @param stxAmount - the amount of stacks (or microstacks) to convert to a USD price
 * @param stxPrice - the current USD price of BASE
 * @param isInMicroBase - if true, the stxAmount is in microstacks
 *
 * @returns string - the formatted current USD price of the given BASE
 */
export const getUsdValue = (
  ...
  const amountInStx = isInMicroStacks ? microToBase(stxAmount) : stxAmount;
 ...
 * stacksToMicro
 *
 * @param {String || Number} amountInBase - the amount of stacks to convert
 */
...
export const getTxTitle = (transaction: Transaction | MempoolTransaction) => {
  switch (transaction.tx_type) {
    ...
    case 'token_transfer':
      return `${microToStacksFormatted(transaction.token_transfer.amount)} BASE transfer`;
    case 'coinbase':
