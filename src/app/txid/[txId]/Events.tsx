...
const getAssetAmounts = (event: TransactionEvent) => {
  switch (event.event_type) {
    ...
    case 'stx_asset':
      return event.asset.amount ? `${microToStacksFormatted(event.asset.amount)} BASE` : undefined;
  }
};
...
const getName = (event: TransactionEvent) => {
  ...
  switch (event.event_type) {
    case 'stx_lock':
      return `${microToStacksFormatted(event.stx_lock_event.locked_amount)} BASE`;
    case 'smart_contract_log':
      return (
        <Code
    ...
    case 'stx_asset':
      return event.asset?.value
        ? `${microToStacksFormatted(event.asset?.value)} BASE`
        : 'BASE transfer';
    default:
