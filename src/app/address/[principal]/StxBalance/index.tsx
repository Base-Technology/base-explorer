...
import { microToBase } from '../../../../common/utils/utils';
...
function StxBalanceBase({ address }: StxBalanceProps) {
  ...
  const totalBalance = microToBase(stxBalance + tokenOfferingLocked);
  const availableBalance = microToBase(stxBalance - locked);
  const stackedBalance = microToBase(locked);
  const minerRewardsBalance = microToBase(minerRewards);
  ...
  return (
    <Section title={qrShowing ? 'Address QR code' : 'Base Balance'} topRight={TopRight}>
      {!qrShowing ? (
        <Stack
          sx={{
            '& > *:not(:last-child)': {
        ...
        >
          ...
          {!!tokenOfferingData ? (
            <Stack gap={2} py={4}>
              ...
              <BalanceItem balance={microToBase(tokenOfferingData.total_locked)} />
              <Button
...
export function StxBalance(props: StxBalanceProps) {
  return (
    <ExplorerErrorBoundary Wrapper={Section} wrapperProps={{ title: 'BASE Balance' }} tryAgainButton>
