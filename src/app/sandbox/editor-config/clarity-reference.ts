export const clarity = {
  functions: [
    ...
    },
    {
      ...
      description:
        "`stx-get-balance` is used to query the BASE balance of the `owner` principal.\n\nThis function returns the BASE balance of the `owner` principal. In the event that the `owner`\nprincipal isn't materialized, it returns 0.\n",
      example:
    ...
    },
    {
      ...
      description:
        "`stx-burn?` debits the `sender` principal's BASE holdings by `amount`, destroying\nthe BASE. The `sender` principal _must_ be equal to the current context's `tx-sender`.\n\nThis function returns (ok true) if the transfer is successful. In the event of an unsuccessful transfer it returns\none of the following error codes:\n\n`(err u1)` -- `sender` does not have enough balance to transfer\n`(err u3)` -- amount to send is non-positive\n`(err u4)` -- the `sender` principal is not the current `tx-sender`\n",
      example:
  ...
  keywords: [
    {
      ...
      example:
        '(print contract-caller) ;; Will print out a Base address of the transaction sender',
    },
    {
      name: 'tx-sender',
      output_type: 'principal',
      ...
      example: '(print tx-sender) ;; Will print out a Base address of the transaction sender',
    },
    {
      name: 'block-height',
      output_type: 'uint',
    ...
    },
    {
      ...
      output_type: 'uint',
      description:
        'Returns the total number of micro-BASE (uBASE) that are liquid in the system as of this block.',
      example: '(print stx-liquid-supply) ;; Will print out the total number of liquid uBASE',
    },
    {
      name: 'is-in-regtest',
      output_type: 'bool',
