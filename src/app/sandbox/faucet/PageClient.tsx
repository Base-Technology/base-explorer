...
const Faucet: NextPage = () => {
  ...
  return (
    <Stack alignItems={'center'} mt={46} gap={6}>
      ...
      <Title>BASE Faucet</Title>
      <Text fontSize={'sm'}>Need BASE to test the network? The faucet can top you up!</Text>
      {!!errorMessage ? <Text color={'error'}>{errorMessage}</Text> : null}
      {isSuccess ? (
        <HStack gap={4} fontSize={'sm'}>
          <Text>💰</Text>
          <Text>BASE coming your way shortly!</Text>
          <Text>💰</Text>
        </HStack>
      ) : null}
      <Stack gap={4}>
